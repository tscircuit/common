...
import {pacmanPolygonPoints, PCMN1, PCMN2, PCMN3, PCMN4} from "./pacmanPolygon"
...
export const ViaGridBoard = ({ children, ...rest }: ViaGridBoardProps) => {
  ...
  const { name: _, ...chipRest } = chipProps
    ...
    <chip name={"PCMN"+cell.index}  pcbX={cell.center.x} pcbY={cell.center.y}
    ...
    {horizontalEdgeViaGridCells.map((cell) => (
      <ViaGridVia pcbX={cell.center.x} pcbY={cell.center.y} key={cell.index} />
    ))}
    ...
    <silkscreentext text="VIAGRID TOP" fontSize="1.5mm" pcbX={50} pcbY={2.5}/>
